...
-- 1: save order by sql file as Functions
-- 2: 
...
-- 4:
SELECT *, DATEDIFF(CURDATE(), hire_date) AS days_employed FROM employees WHERE birth_date LIKE '%12-25' AND hire_date LIKE '199%';
-- 5: 
SELECT MIN(salary) FROM employees.salaries;
SELECT MAX(salary) FROM employees.salaries;
-- 6:
USE employees;
SELECT *, LOWER(CONCAT(SUBSTR(first_name, 1,1), SUBSTR(last_name, 1, 4), '_', SUBSTR(birth_date, 6, 2), SUBSTR(birth_date, 3,2))) AS username FROM employees LIMIT 10;